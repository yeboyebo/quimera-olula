...
export const getAccion = async (id: string): Promise<Accion> =>
    await RestAPI.get<{ datos: Accion }>(`${ApiUrls.CRM_ACCION}/${id}`).then((respuesta) => respuesta.datos);

export const getAcciones = async (
    filtro: Filtro,
    orden: Orden,
    paginacion?: Paginacion
): Promise<RespuestaLista<Accion>> => {
    const q = criteriaQuery(filtro, orden, paginacion);
    return await RestAPI.get<{ datos: Accion[]; total: number }>(ApiUrls.CRM_ACCION + q);
};

export const postAccion = async (accion: Partial<Accion>): Promise<string> => {
    return await RestAPI.post(ApiUrls.CRM_ACCION, accion, "Error al guardar acción").then((respuesta) => respuesta.id);
};
...
export const patchAccion = async (id: string, accion: Partial<Accion>): Promise<void> => {
    ...
    await RestAPI.patch(`${ApiUrls.CRM_ACCION}/${id}`, accionSinNulls, "Error al guardar acción");
};

export const deleteAccion = async (id: string): Promise<void> =>
    await RestAPI.delete(`${ApiUrls.CRM_ACCION}/${id}`, "Error al borrar acción");

export const finalizarAccion = async (id: string): Promise<void> => {
    await RestAPI.patch(`${ApiUrls.CRM_ACCION}/${id}/finalizar`, {}, "Error al finalizar acción");
};