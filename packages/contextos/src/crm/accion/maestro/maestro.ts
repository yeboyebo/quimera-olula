...
const conAcciones =
    (fn: ProcesarListaActivaEntidades<Accion>) =>
        (ctx: ContextoMaestroAcciones) => ({ ...ctx, acciones: fn(ctx.acciones) });

export const Acciones = accionesListaActivaEntidades(conAcciones);
