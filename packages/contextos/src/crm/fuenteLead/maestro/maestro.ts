...
const conFuentesLead =
    (fn: ProcesarListaActivaEntidades<FuenteLead>) =>
        (ctx: ContextoMaestroFuentesLead) => ({ ...ctx, fuentes_lead: fn(ctx.fuentes_lead) });

export const FuentesLead = accionesListaActivaEntidades(conFuentesLead);
