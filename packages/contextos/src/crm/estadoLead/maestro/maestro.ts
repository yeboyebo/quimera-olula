...
const conEstadosLead =
    (fn: ProcesarListaActivaEntidades<EstadoLead>) =>
        (ctx: ContextoMaestroEstadosLead) => ({ ...ctx, estados_lead: fn(ctx.estados_lead) });

export const EstadosLead = accionesListaActivaEntidades(conEstadosLead);
