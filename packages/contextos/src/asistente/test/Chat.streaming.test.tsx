...
import {
  ...
  useAsistenteContext,
} from "#/asistente/vistas/AsistenteRuntimeProvider.tsx";
...
vi.mock("#/asistente/infraestructura.ts", () => ({
  ...
  obtenerMensajesHilo: vi.fn(async () => ({
    threadId: "hilo-1",
    mensajes: [],
  })),
...
function Probe() {
  ...
  return (
    <div>
      <button type="button" onClick={() => setStreamingEnabled(true)}>
        activar-streaming
      </button>
      <button
        type="button"
        onClick={() =>
          enviarMensaje("crea el pedido", [
            {
              nombre: "pedido.xlsx",
...
test("[asistente-chat-stream-05] el id de adjunto devuelto en fin se aplica al mensaje de usuario", async () => {
  ...
  await userEvent.click(
    screen.getByRole("button", { name: "activar-streaming" })
  );
  await userEvent.click(
    screen.getByRole("button", { name: "enviar-con-adjunto" })
  );
