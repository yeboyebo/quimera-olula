...
function ListItemMiLineaAlbaran({ selected = false, funPrimaryLeft, funPrimaryRight, hideSecondary = false, model, modelName, ...props }) {
  const [_, dispatch] = useStateValue();
  const classes = useStyles();
  const linea = model;

  return (
    <QListItemModel modelName={modelName} model={linea} selected={false}>
      <ListItemAvatar>
        ...
        secondary={
          !hideSecondary && (
            <Box width={1} mt={0.5} display="flex" justifyContent="space-between">
              <Box display="inline">
                <Typography component="span" variant="body2" color="textPrimary">{`${linea.referencia ?? ""
                  }`}</Typography>
              </Box>
              <Box display="inline">
...
export default ListItemMiLineaAlbaran;
