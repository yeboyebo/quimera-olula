...
function LineaFacturaCliNueva({ callbackGuardada, idFactura, useStyles }) {
  const [{ inline, linea }, dispatch] = useStateValue();
  const classes = useStyles();
  const schema = getSchemas().lineasFacturasCli;
  useEffect(() => {
    // console.log("INIT NUEVA", idFactura, dispatch, callbackGuardada);
    dispatch({
      type: "onInit",
      payload: {
        idParentLinea: idFactura,
      },
    });
  }, [idFactura]);
  ...
  return (
    <Quimera.Template id="LineaFacturaCliNueva">
      {inline && (
        <Box>
...
export default LineaFacturaCliNueva;
