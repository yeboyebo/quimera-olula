...
function ListItemMisPedidos({ renderAvatar, pedido, selected = false, funSecondaryLeft, avatar = "P", ...props }) {
        ...
        primary={
          <React.Fragment>
            <Box width={1} display="flex" justifyContent="space-between">
              <Box display="inline">{`${pedido.referencia || "SIN REFERENCIA"}`}</Box>
              <Box display="inline">{`${util.euros(pedido.total)}`}</Box>
            </Box>
          </React.Fragment>
        }
        secondary={
